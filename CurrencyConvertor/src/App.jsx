...
import useGaurav from './hooks/gaurav'
import './App.css'
...
function App() {
  ...
  const Gaurav  = useGaurav(from);
  const options = Object.keys(Gaurav);
  ...
  const convert = () => {
    setConvertedAmount(amount * Gaurav[to]);
  }
      ...
      <div className="w-full">
        ...
        <form onSubmit={(e) => {
          ...
          <div className="w-full mb-1">
            <InputBox label="from" amount={amount} currencyOptions={options} onCurrencyChange={(currency) => setAmount(currency)} selectCurrency={from} onAmountChange={(amount) => {
              setAmount(amount);
            }}/>
          </div>
